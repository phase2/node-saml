...
exports.create = function(options, callback) {
  ...
  // This is actually the signature being created here
  ...
  if (options.audiences) {
    var audienceRestriction = doc.createElementNS(NAMESPACE, 'AudienceRestriction');
  ...
  return new Promise(async (resolve, reject) => {
    xmlenc.encrypt(signed, encryptOptions, function (err, encrypted) {
      if (err) {
        reject(new Error(`Error creating SAML ${err}`));
      }
      encrypted = `<EncryptedAssertion xmlns="urn:oasis:names:tc:SAML:2.0:assertion">${encrypted}</EncryptedAssertion>`;
      resolve(utils.removeWhitespace(encrypted));
    });
  });
};
